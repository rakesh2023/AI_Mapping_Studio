"""Per-client BillingCenter dictionary index.

Thin wrapper over cc_dictionary_service: BillingCenter uses the SAME Guidewire
`entityModel.xml` parser (physical bc_/bctl_ table names come straight from the XML),
so the only difference is which table set the rows live in — the `bc_dict_*` tables.
Every function delegates to cc_dictionary_service with prefix="bc_dict".

Built when a client's Product is Billing and they upload a BillingCenter dictionary .zip on
Product Data Dictionary. Powers the Data Reconciliation page's grounded NL->SQL for the
"billingcenter" source.
"""
